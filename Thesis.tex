...
six-degree-of-freedom motion simulator.\medskip{}
...
\noindent We derive its forward and inverse kinematics to verify that
the required workspace can be reached, then formulate the inverse
dynamics to assess 

\noindent whether off-the-shelf servomotors can meet the dynamic performance
requirements.\medskip{}

\noindent This analysis serves two purposes: it identifies which servos
are capable of the task, and it establishes whether advancing to a
full Stewart-platform architecture is justified.\medskip{}
...
in each axis along with fundamental associated frequencies.\medskip{}
...
\noindent\begin{equation}
\phi = \arcsin\left( \frac{\cos\theta}{n} \right)
\end{equation}
...
of motor driving the crank is $J$. At this stage of the analysis
we assume the crank arm and the connecting rod has no mass. In the
$theta$ as the generalized co-ordinate system.
...
\noindent\begin{equation}
\mathcal{L}(\theta, \dot{\theta}) = \tfrac{1}{2} \left( J + m \left( \tfrac{dz}{d\theta} \right)^2 \right) \dot{\theta}^2 - m g z(\theta)
\label{eq:L}
\end{equation}
...
$q=\theta$, and thus required torque becomes $Q=\tau$ .

\noindent Substituting we get:\medskip{}
...
&= \frac{d}{dt}\Bigl[(J + m\,(z'(\theta))^2)\,\dot{\theta}\Bigr]
  - \Bigl[\tfrac12\bigl(2m\,z'(\theta)\,z''(\theta)\bigr)\,\dot{\theta}^2
           - m g\,z'(\theta)\Bigr] \\[8pt]
&= (J + m\,(z'(\theta))^2)\,\ddot{\theta}
  + 2m\,z'(\theta)\,z''(\theta)\,\dot{\theta}^2
  - \bigl(m\,z'(\theta)\,z''(\theta)\,\dot{\theta}^2 
         - m g\,z'(\theta)\bigr) \\[6pt]
&= (J + m\,(z'(\theta))^2)\,\ddot{\theta}
  + m\,z'(\theta)\,z''(\theta)\,\dot{\theta}^2
  + m g\,z'(\theta)\,.
\end{aligned}
\label{eq:ID}
\end{equation}Here,
